...
import { collections, getImageURL } from './components/strapi/strapi_interface';
...
function App() {
  ...
  useEffect(() => {
    ...
    fetch(collections.tests)
      .then(x => x.json())
      .then(x => set_test_gallery(x.data.map((x: test_entry) => x.attributes.gallery.data[0].attributes.url)));
  }, []);
